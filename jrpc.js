...
var { pinterestVideo } = require('./modulos-api/scrapperlinda.js')
var TikTokScraper = require('tiktok-scraper');
var download = require('@phaticusthiccy/open-apis')
var thiccysapi = require('textmaker-thiccy');
var { pinterestimg } = require('./lib/funcoes.js')
...
const banner = cfonts.render(("GOJO REST APIS"), {
font: "block",
align: "center",
...
await listkeys(apikey, req);
url = req.query.url
 if (!url) return res.json({ status : false, criador : `@Klaus`, mensagem : "Coloque o parametro: url"})
fbdown(url).then(data => {
res.json({
status: true,
mensagem: 'GOJO REST APIS - MELHOR SITE DE APIS',
criador: `@Klaus - 68 99309374`,
resultado: data
})}).catch(e => {
res.json({
...
const { instagram } = require('betabotz-tools'); // Import 

app.all('/api/instamp4', async (req, res) => {
    ...
    if (!apikey) {
        return res.json({ resultado: 'Cade o parametro apikey?' });
    }

    const apikeyIndex = key.map(i => i.apikey).indexOf(apikey);
    if (apikeyIndex === -1 || key[apikeyIndex].request <= 0) {
        return res.sendFile(path.join(__dirname, "./public/", "limited.html"));
    }

    await listkeys(apikey, req);

    try {
        const results = await instagram(url); // Use betabotz-tools to scrape Instagram
        res.json({
            status: true,
            mensagem: 'GOJO REST APIS - MELHOR SITE DE APIS',
            criador: `@Klaus`,
        ...
        console.log(err);
        res.status(500).json({ status: false, mensagem: 'Erro ao processar a requisição' });
    }
});

const { pinterest } = require('betabotz-tools'); // Import 

app.all('/api/pinterest_mp4', async (req, res) => {
const url = req.query.url;
if (!url) {
return res.json({
status: false,
...
try {
const results = await pinterest(url); // Use betabotz-tools to scrape pinterest
res.json({
status: true,
mensagem: 'GOJO REST APIS - MELHOR SITE DE APIS',
criador: `@Klaus`,
...
res.json({
status: true,
mensagem: 'GOJO REST APPIS - MELHOR SITE DE APIS',
criador: `@Klaus`,
resultado: {
...
if(!key.map(i => i.apikey)?.includes(apikey))return res.sendFile(path.join(__dirname, "./public/", "limited.html"))
await listkeys(apikey, req);
if (!q) return res.json({ status : false, creator : `@Klaus`, message : "Cade o parametro q?"}) 
...
await listkeys(apikey, req);
q = req.query.q
 if (!q) return res.json({ status : false, criador : `@Klaus`, mensagem : "Coloque o parametro: q"})
...
await listkeys(apikey, req);
nome = req.query.nome
 if (!nome) return res.json({ status : false, criador : `@Klaus`, mensagem : "Coloque o parametro: nome"})
...
await listkeys(apikey, req);
signo = req.query.signo
if (!signo) return res.json({ status : false, criador : `@Klaus`, mensagem : "Coloque o parametro: signo"})
...
categoria = req.query.categoria
 if (!categoria) return res.json({ status : false, criador : `@Klaus`, mensagem : "Coloque o parametro: categoria"})
...
pergunta = req.query.pergunta
 if (!pergunta) return res.json({ status : false, criador : `@Klaus`, mensagem : "Coloque o parametro: pergunta"})
...
texto = req.query.texto
 if (!texto) return res.json({ status : false, criador : `@Klaus`, mensagem : "Coloque o parametro: texto"})
...
if(!key.map(i => i.apikey)?.includes(apikey))return res.sendFile(path.join(__dirname, "./public/", "limited.html"))
await listkeys(apikey, req);
  if (!query) return res.json({ status : false, criador : `@Klaus`, mensagem : "Coloque o parametro: query"})
...
if (!url) return res.json({ status : false, creator : `@Klaus`, message : "Cade o parametro url?"}) 
        if(key.includes(apikey)){
       igstory(url)
	.then(data => {
		var result = data;
...
await listkeys(apikey, req);
text = req.query.text
if (!text) return res.json({ status : false, creator : `@Klaus`, message : "Cade o parametro text?"})
        if(key.includes(apikey)){
...
await listkeys(apikey, req);
query = req.query.query
if (!query) return res.json({ status : false, creator : `@Klaus`, message : "Cade o parametro query?"})
wikimedia(query)
.then(hasil => {
var resultado = hasil;
res.json({
...
await listkeys(apikey, req);
  try {
    ...
    return res.json({ DONO: "KLAUS - 68999309378", ...randomAccount, "total": '🩸 | *QUANTIDADE RESTANTE:* ' + totalAccounts, "mensagem": '❗ | *OBS:* ' + 'Não Mude nenhuma senha, eu saberei que foi vc!' })
  } catch (error) {
...
await listkeys(apikey, req);
url = req.query.url
if (!url) return res.json({ status : false, criador : `@Klaus`, mensagem : "Coloque o parametro: q"})
...
auu = await Request(`https://api.bronxyshost.com.br/api-bronxys/letra_musica?letra=${query}&apikey=948743jdgk`)
res.json({
status: true,
mensagem: 'GOJO REST APPIS - MELHOR SITE DE APIS',
criador: `@Klaus`,
resultado: {
titulo: `${abc.titulo}`,
artista: `${abc.compositor}`,
LetraMusic: `${abc.letra}`
}
})
})
...
if(!key.map(i => i.apikey)?.includes(apikey))return res.sendFile(path.join(__dirname, "./public/", "limited.html"))
await listkeys(apikey, req);
wikip = await axios.get(`https://pt.wikipedia.org/tiktok/api.php?action=query&format=json&list=search&srsearch=${encodeURIComponent(q)}&prop=info&inprop=url`);
wikis = await axios.get(`https://pt.wikipedia.org/tiktok/api.php?format=json&action=query&prop=extracts&exintro&explaintext&redirects=1&pageids=${wikip.data.query.search[0].pageid}`);
res.json({
status: true,
mensagem: 'GOJO REST APPIS - MELHOR SITE DE APIS',
criador: `@Klaus`,
...
app.get('/ias/gpt4', async (req, res) => {
  const { q } = req.query;

  if (!q) {
    return res.json({ msg: 'Olá, em que posso te ajudar? Pergunte e eu responderei 🙂' });
  }
  ...
  const hora120 = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });

  const messages = [
    { role: 'system', content: `Caso perguntem, a data de hoje é ${date} e a hora é ${hora120}. Ao enviar a hora use somente as horas e os minutos, ignore os segundos.` },
    ...chatHistory
  ];
...
.then(async (wikip) => {
const wikis = await axiosapp.get(`https://pt.wikipedia.org/tiktok/api.php?format=json&action=query&prop=extracts&exintro&explaintext&redirects=1&pageids=${wikip[0].pageid}`)
...
status = {
    ...
    request: totalreq,
        ...
        info:{       
            owner: 'Klaus',
            whatsapp: 'https://wa.me/556899309378'
        }
    }
res.json(status)
}});
